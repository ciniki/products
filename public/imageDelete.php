...
function ciniki_products_imageDelete(&$ciniki) {
    ...
    $rc = ciniki_products_checkAccess($ciniki, $args['tnid'], 'ciniki.products.imageDelete', 0); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }   
    ...
    if( !isset($rc['item']) ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.products.66', 'msg'=>'Product image does not exist'));
    }
    $item = $rc['item'];
    ...
    return ciniki_core_objectDelete($ciniki, $args['tnid'], 'ciniki.products.image', $args['product_image_id'], $item['uuid'], 0x07);
}
?>
