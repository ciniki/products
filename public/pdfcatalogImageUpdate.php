...
function ciniki_products_pdfcatalogImageUpdate(&$ciniki) {
    ...
    $rc = ciniki_products_checkAccess($ciniki, $args['business_id'], 'ciniki.products.pdfcatalogImageUpdate');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    $rc = ciniki_core_objectUpdate($ciniki, $args['business_id'], 'ciniki.products.pdfcatalogimage', $args['catalog_image_id'], $args, 0x04);
    if( $rc['stat'] != 'ok' ) {
    ...
    ciniki_core_hookExec($ciniki, $args['business_id'], 'ciniki', 'web', 'indexObject', array('object'=>'ciniki.products.pdfcatalogImage', 'object_id'=>$args['catalog_image_id']));
