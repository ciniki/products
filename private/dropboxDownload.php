...
function ciniki_products_dropboxDownload(&$ciniki, $tnid) {
    ...
    if( !isset($rc['settings']['dropbox-products']) || $rc['settings']['dropbox-products'] == '') {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.products.26', 'msg'=>'Dropbox products not setup.'));
    }
    ...
    if( !isset($rc['settings']['apis-dropbox-access-token']) 
        ...
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.products.27', 'msg'=>'Dropbox not configured.'));
    }
    ...
    foreach($updates as $product_code => $product) {
        ...
        foreach($product as $field => $details) {
            ...
            elseif( ($field == 'synopsis' || $field == 'description' ) && $details['mime_type'] == 'application/rtf' ) {
                ...
                if( $field == 'synopsis' ) {
                    if( $rc['content'] != $ciniki_product['short_description'] ) {
                        $update_args['short_description'] = $rc['content'];
                    }
                } elseif( $field == 'description' ) {
                    if( $rc['content'] != $ciniki_product['long_description'] ) {
