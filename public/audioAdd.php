...
function ciniki_products_audioAdd(&$ciniki) {
    ...
    $rc = ciniki_products_checkAccess($ciniki, $args['tnid'], 'ciniki.products.audioAdd', $args['product_id']); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    } 
    ...
    $rc = ciniki_core_objectAdd($ciniki, $args['tnid'], 'ciniki.products.audio', $args, 0x04);
    if( $rc['stat'] != 'ok' ) {
    ...
    if( isset($args['sequence']) && $adjust_sequence == 'yes' ) {
        ...
        $rc = ciniki_products_audioUpdateSequences($ciniki, $args['tnid'], $audio_id, 
            $args['sequence'], -1);
        if( $rc['stat'] != 'ok' ) {
            ciniki_core_dbTransactionRollback($ciniki, 'ciniki.products');
            return $rc;
