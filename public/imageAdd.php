...
function ciniki_products_imageAdd(&$ciniki) {
    ...
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.products.63', 'msg'=>'Unable to get a new UUID', 'err'=>$rc['err']));
    }
    $args['uuid'] = $rc['uuid'];
    ...
    if( $rc['num_rows'] > 0 ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.products.64', 'msg'=>'You already have an image with this name, please choose another name'));
    }


    if( $args['product_id'] <= 0 ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.products.65', 'msg'=>'No product specified'));
    }
