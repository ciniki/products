...
function ciniki_products_pdfcatalogUpdate(&$ciniki) {
    ...
    $rc = ciniki_products_checkAccess($ciniki, $args['tnid'], 'ciniki.products.pdfcatalogUpdate');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    $rc = ciniki_core_objectUpdate($ciniki, $args['tnid'], 'ciniki.products.pdfcatalog', $args['catalog_id'], $args, 0x04);
    if( $rc['stat'] != 'ok' ) {
...
//    ciniki_core_hookExec($ciniki, $args['tnid'], 'ciniki', 'web', 'indexObject', array('object'=>'ciniki.products.pdfcatalog', 'object_id'=>$args['catalog_id']));
